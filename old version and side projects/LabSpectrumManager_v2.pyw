...
class LabSpectrumManager:
    def __init__(self, root):
        ...
        # Pulsante correzione scattering — riga separata per leggibilità
    ...
    def apri_correzione_scattering(self):
        """Apre una finestra di correzione scattering sullo spettro selezionato.
        Formula: correction = (A * 10^VS) * (lambda/1000)^(-P) + Offset
        Il risultato viene aggiunto come nuovo spettro <nome>_corr.
        Non applicabile a spettri FTIR.
        """
        ...
        x_data = self.spectra[name]['df'].index.to_numpy(dtype=float)
        y_data = self.spectra[name]['df'][name].to_numpy(dtype=float)
        tipo   = self.spectra[name]['info']['Type']

        # --- Toplevel ---
        top = tk.Toplevel(self.root)
        top.title(f"Scattering Correction — {name}")
        top.geometry("920x640")
        top.grab_set()

        # Figura anteprima
        fig_sc = plt.Figure(figsize=(8, 4))
        ax_sc  = fig_sc.add_subplot(111)
        ax_sc.grid(True, alpha=0.3)
        line_orig, = ax_sc.plot(x_data, y_data,              color='steelblue', alpha=0.5, lw=1.5, label='Original')
        line_corr, = ax_sc.plot(x_data, np.zeros_like(y_data), color='orange',  lw=1.2, ls='--',  label='Scattering')
        line_res,  = ax_sc.plot(x_data, y_data.copy(),        color='green',    lw=2,              label='Corrected')
        ax_sc.legend(fontsize=8)
        ax_sc.set_xlabel("Wavelength (nm)")
        ax_sc.set_ylabel("Intensity (A.U.)" if tipo == 'Fluorescence' else "Absorbance (A)")

        canvas_sc = FigureCanvasTkAgg(fig_sc, master=top)
        canvas_sc.get_tk_widget().pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        NavigationToolbar2Tk(canvas_sc, top)

        # Controlli
        ctrl = tk.Frame(top, bg='#f0f4f7')
        ctrl.pack(fill=tk.X, padx=15, pady=6)

        def make_slider(parent, label, from_, to, init, resolution):
            row = tk.Frame(parent, bg='#f0f4f7')
            row.pack(fill=tk.X, pady=2)
            tk.Label(row, text=label, width=10, anchor='w', bg='#f0f4f7',
                     font=('Consolas', 9)).pack(side=tk.LEFT)
            var = tk.DoubleVar(value=init)
            tk.Scale(row, variable=var, from_=from_, to=to, resolution=resolution,
                     orient=tk.HORIZONTAL, length=450, bg='#f0f4f7',
                     font=('Consolas', 9), showvalue=True).pack(side=tk.LEFT)
            return var

        var_A   = make_slider(ctrl, 'A',       0.0,  0.01, 0.0005, 0.00001)
        var_VS  = make_slider(ctrl, 'VS (log)', -2.0,  2.0, 0.0,    0.01)
        var_OFF = make_slider(ctrl, 'Offset',  -0.5,  0.5, 0.0,    0.001)

        # Esponente P — RadioButton Tk
        p_row = tk.Frame(ctrl, bg='#f0f4f7')
        p_row.pack(fill=tk.X, pady=2)
        tk.Label(p_row, text='P', width=10, anchor='w', bg='#f0f4f7',
                 font=('Consolas', 9)).pack(side=tk.LEFT)
        var_P = tk.IntVar(value=2)

        def _on_p_change():
            _aggiorna_preview()

        for p_val in (2, 3, 4):
            tk.Radiobutton(p_row, text=str(p_val), variable=var_P, value=p_val,
                           bg='#f0f4f7', command=_on_p_change).pack(side=tk.LEFT, padx=8)

        def _aggiorna_preview(*_):
            A, VS, P, OFF = var_A.get(), var_VS.get(), var_P.get(), var_OFF.get()
            w_norm     = x_data / 1000.0
            correction = (A * (10 ** VS)) * (w_norm ** -P) + OFF
            y_corr     = y_data - correction
            line_corr.set_ydata(correction)
            line_res.set_ydata(y_corr)
            y_all   = np.concatenate([y_data, y_corr])
            margin  = (y_all.max() - y_all.min()) * 0.05 or 0.05
            ax_sc.set_ylim(y_all.min() - margin, y_all.max() + margin)
            canvas_sc.draw_idle()

        var_A.trace_add('write', _aggiorna_preview)
        var_VS.trace_add('write', _aggiorna_preview)
        var_OFF.trace_add('write', _aggiorna_preview)

        # Pulsanti Apply / Cancel
        btn_row = tk.Frame(top, bg='#f0f4f7')
        ...
        def _applica():
            ...
            correction = (A * (10 ** VS)) * (w_norm ** -P) + OFF
            y_corr     = y_data - correction
            new_name   = f"{name}_corr"
            new_info   = dict(self.spectra[name]['info'])
            new_info['Sample']         = new_name
            new_info['Derived from']   = name
            new_info['Scatter A']      = f"{A:.5f}"
            new_info['Scatter VS']     = f"10^{VS:.2f}"
            new_info['Scatter P']      = str(P)
            new_info['Scatter Offset'] = f"{OFF:.3f}"
            self.spectra[new_name] = {
                'df':   pd.DataFrame({'x': x_data, new_name: y_corr}).set_index('x'),
                'info': new_info,
            }
            top.destroy()
            self.aggiorna_vista()

        tk.Button(btn_row, text="Apply & Close", command=_applica,
